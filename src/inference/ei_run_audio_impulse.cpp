...
static void display_results(ei_impulse_result_t* result)
{
    ...
    uint8_t s_buf[1];
    uint8_t ix_max;
    float ix_max_val = -1.0f;

    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {            
        if (result->classification[ix].value > ix_max_val) {
            ix_max_val = result->classification[ix].value;
            ix_max = ix;
        }
    } 
    ...
    if (ix_max == 0  && ix_max_val > 0.75) {
         ...
         if (count >= 5) {
             ei_printf("Running Faucet\n");
             if (detection.running_faucet == false) { 
                 detection.running_faucet = true;
                 detection.start_ms = ei_read_timer_ms();
             }

             if ((detection.running_faucet == true) && ((ei_read_timer_ms() - detection.start_ms) > 5000)) {
                 buzzer_on = true;
         ...
         ei_printf("Noise\n");
         dk_set_led(LED_GREEN, 0); 
         if (detection.running_faucet == true) {
             detection.running_faucet = false;
             buzzer_on = false;
    ...
    if(dev->get_serial_channel() == UART) {
        ...
        if (buzzer_on) {
            cJSON_AddStringToObject(response, "alert", "1");
        } else {
            cJSON_AddStringToObject(response, "alert", "0");
        }
